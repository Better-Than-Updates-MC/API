...
import com.terraformersmc.modmenu.gui.entries.ChildEntry;
import com.terraformersmc.modmenu.gui.entries.IndependentEntry;
import com.terraformersmc.modmenu.gui.entries.ParentEntry;
import com.terraformersmc.modmenu.util.ModListSearch;
import com.terraformersmc.modmenu.util.TestModContainer;
		...
		for (int index = 0; index < itemCount; ++index) {
			...
			if (entryBottom >= this.top && entryTop <= this.bottom) {
				...
				if (this.renderSelection && this.isSelectedItem(index)) {
					...
					tessellator_1.start();
					tessellator_1.addVertex(entryLeft, (entryTop + entryHeight + 2), 0.0D);
					tessellator_1.addVertex(selectionRight, (entryTop + entryHeight + 2), 0.0D);
					tessellator_1.addVertex(selectionRight, (entryTop - 2), 0.0D);
					tessellator_1.addVertex(entryLeft, (entryTop - 2), 0.0D);
					tessellator_1.draw();
					GL11.glColor4f(0f, 0f, 0f, 1f);
					tessellator_1.start();
					tessellator_1.addVertex((entryLeft + 1), (entryTop + entryHeight + 1), 0.0D);
					tessellator_1.addVertex((selectionRight - 1), (entryTop + entryHeight + 1), 0.0D);
					tessellator_1.addVertex((selectionRight - 1), (entryTop - 1), 0.0D);
					tessellator_1.addVertex((entryLeft + 1), (entryTop - 1), 0.0D);
					tessellator_1.draw();
