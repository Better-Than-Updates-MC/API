...
import com.terraformersmc.modmenu.util.BadgeRenderer;
...
public class ModListScreen extends Screen {
	...
	public void onMouseEvent() {
		...
		if (dWheel != 0) {
			int mouseX = Mouse.getEventX() * this.width / this.client.actualWidth; // field_6326_c
			int mouseY = this.height - Mouse.getEventY() * this.height / this.client.actualHeight - 1; // field_6325_d
	...
	public void init() {
		...
		this.descriptionListWidget = new DescriptionListWidget(this.client, paneWidth, this.height, paneY + 60, this.height - 36, 9 + 1, this);
		this.descriptionListWidget.setLeftPos(rightPaneX);
		ButtonWidget configureButton = new ModMenuTexturedButtonWidget(CONFIGURE_BUTTON_ID, width - 24, paneY, 20, 20, 0, 0, CONFIGURE_BUTTON_LOCATION, 32, 64, "Configure...") {
			@Override
		...
		ButtonWidget websiteButton = new ButtonWidget(WEBSITE_BUTTON_ID, rightPaneX + (urlButtonWidths / 2) - (cappedButtonWidth / 2), paneY + 36, urlButtonWidths > 200 ? 200 : urlButtonWidths, 20, "Website") {
			@Override
		...
		ButtonWidget issuesButton = new ButtonWidget(ISSUES_BUTTON_ID, rightPaneX + urlButtonWidths + 4 + (urlButtonWidths / 2) - (cappedButtonWidth / 2), paneY + 36, urlButtonWidths > 200 ? 200 : urlButtonWidths, 20, "Issues") {
			@Override
			public void render(Minecraft mc, int var1, int var2) {
				visible = selected != null; // visible = selected != null
		...
		this.buttons.add(new ButtonWidget(MODS_FOLDER_BUTTON_ID, this.width / 2 - 154, this.height - 28, 150, 20, "Open Mods Folder"));
		this.buttons.add(new ButtonWidget(DONE_BUTTON_ID, this.width / 2 + 4, this.height - 28, 150, 20, "Done"));
	...
	protected void mouseReleased(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == -1) {
			int mouseDX = Mouse.getEventDX() * this.width / this.client.actualWidth; // field_6326_c
			int mouseDY = this.height - Mouse.getEventDY() * this.height / this.client.actualHeight - 1; // field_6325_d
	...
	public void render(int mouseX, int mouseY, float delta) {
		...
		this.drawTextWithShadowCentred(textRenderer, this.textTitle, this.modList.getWidth() / 2, 8, 0xffffff);
		super.render(mouseX, mouseY, delta);
		if (showModCount || !filterOptionsShown) {
			textRenderer.drawText("Showing " + NumberFormat.getInstance().format(modList.getDisplayedCount()) + "/" + NumberFormat.getInstance().format(FabricLoader.getInstance().getAllMods().size()) + " Mods", searchBoxX, 52, 0xFFFFFF);
		}
		if (selectedEntry != null) {
			...
			if (textRenderer.getTextWidth(name) > maxNameWidth) {
				int maxWidth = maxNameWidth - textRenderer.getTextWidth("...");
				trimmedName = "";
				while (textRenderer.getTextWidth(trimmedName) < maxWidth && trimmedName.length() < name.length()) {
					trimmedName += name.charAt(trimmedName.length());
				}
				trimmedName = trimmedName.isEmpty() ? "..." : trimmedName.substring(0, trimmedName.length() - 1) + "...";
			}
			textRenderer.drawText(trimmedName, x + imageOffset, paneY + 1, 0xFFFFFF);
			if (mouseX > x + imageOffset && mouseY > paneY + 1 && mouseY < paneY + 1 + 9 && mouseX < x + imageOffset + textRenderer.getTextWidth(trimmedName)) {
				setTooltip("Mod ID: " + metadata.getId());
			}
			if (init || badgeRenderer == null || badgeRenderer.getMetadata() != metadata) {
				badgeRenderer = new BadgeRenderer(client, x + imageOffset + textRenderer.getTextWidth(trimmedName) + 2, paneY, width - 28, selectedEntry.container, this);
				init = false;
			}
			badgeRenderer.draw(mouseX, mouseY);
			textRenderer.drawText("v" + metadata.getVersion().getFriendlyString(), x + imageOffset, paneY + 2 + lineSpacing, 0x808080);
			String authors;
			...
			if (!names.isEmpty()) {
				...
				RenderUtils.INSTANCE.drawWrappedString(textRenderer, "By " + authors, x + imageOffset, paneY + 2 + lineSpacing * 2, paneWidth - imageOffset - 4, 1, 0x808080);
			}
	...
	public void overlayBackground(int x1, int y1, int x2, int y2, int red, int green, int blue, int startAlpha, int endAlpha) {
		...
		client.textureManager.bindTexture(client.textureManager.getTextureId("/gui/background.png"));
		GL11.glColor4f(1f, 1f, 1f, 1f);
		tessellator.start();
